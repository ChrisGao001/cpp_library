...
void SkipList<Key,Comparator>::Remove(const Key& key) {
  ...
  logger("Remove key=%d, height=%d",key, height);
  for (int i = 0; i < height; i++) {
    if (prev[i]->Next(i) != x) {
      break;
    }
  ...
  for (int i=height-1; i>=0; --i) {
    if (head_->Next(i) == NULL) {
        --max_height_;
        logger("Remove key=%d, height=%d",key, max_height_);
    }
  }
  --size_;
...
template<typename Key, class Comparator>
void SkipList<Key,Comparator>::Destroy()   {
    Node *cur = head_->Next(0);
    Node *next = NULL;
    while (cur != NULL) {
        next = cur->Next(0);
        logger("destroy 0x%p,value=%d", cur, cur->key);
        free(cur);
        cur  = next;
    }
    free(head_); 
