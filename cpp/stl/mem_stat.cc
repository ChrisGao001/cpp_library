...
int main() {
  ...
  assert(getMemUsageInBytes() == 0);
  hashmap m;
  for(uint32_t k = 0; k < N; k++) m[k]=k;
  std::cout << "memory usage per element of a unordered_map<uint32_t, uint32_t> : " << getMemUsageInBytes() * 1.0 / N << std::endl;

  initializeMemUsageCounter();
  assert(getMemUsageInBytes() == 0);
  mymap mm;
  for(uint32_t k = 0; k < N; k++) mm[k]=k;
  std::cout << "memory usage per element of a map<uint32_t, uint32_t> : " << getMemUsageInBytes() * 1.0 / N << std::endl;

  initializeMemUsageCounter();
  assert(getMemUsageInBytes() == 0);
  hashset h2;
  for(uint32_t k = 0; k < 17000000; k++) h2.insert(k);
  for(uint32_t k = 0; k < 17000000; k++) h2.erase(k);
  std::cout << "hashset size: " << h2.size() << " bucket_count: " << h2.bucket_count() << " mem: " << getMemUsageInBytes() << std::endl;
  h2.rehash(10);
  std::cout << "hashset size: " << h2.size() << " bucket_count: " << h2.bucket_count() << " mem: " << getMemUsageInBytes() << std::endl;
