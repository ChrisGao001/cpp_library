	...
	public:
		...
		void saveToBinaryFile(std::string filename) {
			...
			if (out.is_open()) {
				...
				out.write((char*)&dim1, sizeof(int));
				out.write((char*)&dim2, sizeof(int));
				for (uint i = 0; i < dim1; i++) {
	    ...
	    uint row,col;
            in_file >> row >> col;
            std::cout << "dim1:" << row << " dim2:" << col << std::endl;
            setSize(row, col);
			for (uint i_1 = 0; i_1 < dim1; i_1++) {
				for (uint i_2 = 0; i_2 < dim2; i_2++) {
					T v;
					in_file >> v;
					std::cout << i_1 << "," << i_2 << "," <<  v << std::endl;
					value[i_1][i_2] = v;
				}
			}
			in_file.close();
...
class DMatrixDouble : public DMatrix<float> {
	public:
